...
void renderSpriteProjection(void) {
    ...
    //bubble-sort the sprites by distance
    ...
    for (int i = 0; i < numVisibleSprites; i++) {
        ...
        int textureWidth = upng_get_width(getTexture(sprite.texture));
        int textureHeight = upng_get_height(getTexture(sprite.texture));
        ...
        for (int x = spriteLeftX; x < spriteRightX; x++) {
            ...
            for (int y = spriteTopY; y < spriteBottomY; y++) {
                if (x > 0 && x < WINDOW_WIDTH && y > 0 && y < WINDOW_HEIGHT){
                    ...
                    color_t* spriteTextureBuffer = (color_t*) upng_get_buffer(getTexture(sprite.texture));
